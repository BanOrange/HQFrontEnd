...
function getReport() {
  ...
  axios.post("http://localhost:8080/manager/getReport", qs.stringify(data))
    .then((res) => {
      ...
      var start = res.data.report_start.split("-");
      report_start1.value = start[0]
      report_start2.value = start[1]
      report_start3.value = start[2]

      var end = res.data.report_end.split("-");
      report_end1.value = end[0]
      report_end2.value = end[1]
      report_end3.value = end[2]

      report_income.value = res.data.report_income;
      teacher_num.value = res.data.teacher_num;
      stu_num.value = res.data.stu_num;
      course_num.value = res.data.course_num;
      company_num.value = res.data.company_num;
      executor_num.value = res.data.executor_num;
...
<template>
    ...
    <el-input disabled v-model="report_income" style="width: 100px" />
    <el-text>￥/人</el-text><br><br>
    <el-text>讲师人数：</el-text>
    <el-input disabled v-model="teacher_num" style="width: 100px" /><br><br>
    <el-text>课程数量：</el-text>
    <el-input disabled v-model="course_num" style="width:200px" /><br><br>
    <el-text>学员人数：</el-text><br>
    <el-input disabled v-model="stu_num" style="width:500px" :autosize="{ minRows: 10, maxRows: 100 }" type="textarea"
     /><br><br>
    <el-text>培训公司数量：</el-text>
    <el-input disabled v-model="company_num" style="width: 100px" /><br><br>
    <el-text>执行人数量：</el-text>
    <el-input disabled v-model="executor_num" style="width: 100px" />
    <br><br>
